...
class ProductDisplayItemMain extends React.Component {
            ...
            if (Config.arKit.enable) {
              this.urlExists(Config.arKit.skuArImagesUrl.replace('%sku%', res._code[0].code), (exists) => {
                this.setState({
                  productData: res,
                  arFileExists: exists,
                }, this.trackProductDisplayItemVisit);
  ...
  //   }
  // }
  // trackProductDisplayItemVisit() {
  //   const { productData } = this.state;
  //   const skuCode = productData._code[0].code;
  //   logProductPageVisit(skuCode);
  // }

  initialSelection(productData) {
    let indexSelector = 0;
    if (productData._definition[0]._options) {
      productData._definition[0]._options[0]._element.forEach((selectorName, index) => {
        if (selectorName['display-name'] === 'Size') {
          indexSelector = index;
          this.setState({
            selectionValue: productData._definition[0]._options[0]._element[indexSelector]._selector[0]._chosen[0]._selectaction[0].self.uri,
          });
        }
      });
    }
  ...
  renderProductImage() {
    ...
    const imageSrc = imageUrl || Config.skuImagesUrl.replace('%sku%', productData._code[0].code);
      ...
      if (arBrowserSupported.relList.supports('ar') && !isInStandaloneMode && arFileExists) {
        return (
          <a href={Config.arKit.skuArImagesUrl.replace('%sku%', productData._code[0].code)} rel="ar">
            <img src={imageSrc} onError={(e) => { e.target.src = imgPlaceholder; }} alt={intl.get('none-available')} className="itemdetail-main-img" />
          </a>
        );
      }
    } catch (error) {
      // Do nothing. Error only gets thrown when running storyshots.
    }
    return (
  ...
  render() {
    ...
    if (productData) {
      ...
      const cartBtnOverride = dataSet ? dataSet.cartBtnOverride : '';
        ...
        <div className={`itemdetail-component container-3 ${customClass}`}>
          <div>
            ...
            <div className="itemdetail-details">
              <div className="itemdetail-title-region" data-region="itemDetailTitleRegion" style={{ display: 'block' }}>
                <div>
              ...
              <div className="itemdetail-availability-region" data-region="itemDetailAvailabilityRegion" style={{ display: 'block' }}>
              ...
              <div className="itemdetail-addtocart" data-region="itemDetailAddToCartRegion" style={{ display: 'block' }}>
                <div>
                    ...
                    <div className="form-group-submit">
                      <div className="form-content form-content-submit col-sm-offset-4">
                        <button
                          ...
                          type="submit"
                        >
                          {((cartBtnOverride !== '') ? cartBtnOverride : intl.get('add-to-cart')) }
                        </button>
                      </div>
                    </div>
                  ...
                  {(ProductDisplayItemMain.isLoggedIn(Config) && !Object.keys(productData._addtocartform[0].configuration).length > 0) ? (
