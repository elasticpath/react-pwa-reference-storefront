...
async function loginUserRegister(page, user) {
...
async function navigateToProduct(page, product) {
  ...
  const SUB_CATEGORY_CSS = `${PARENT_CATEGORY_CSS} > ul > li > a[title="${subCategory}"]`;
  ...
  const productLink = await page.$x(`//a[contains(text(), "${name}")]`);
...
async function addProductToCart(page, product) {
  ...
  await navigateToProduct(page, product);
...
async function addPaymentMethodToProfile(page, paymentMethod) {
  const CARD_TYPE = '#CardType';
  const CARD_HOLDER_NAME = '#CardHolderName';
  const CARD_NUMBER = '#CardNumber';
  const EXPIRY_MONTH = '#ExpiryMonth';
