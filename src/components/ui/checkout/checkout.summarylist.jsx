            ...
            return (
                <li className="cart-applied-promotions" data-region="cartAppliedPromotionsRegion">
                    ...
                    {this.props.data['_appliedpromotions'][0]['_element'].map(promotion => {
                        return (
                            <span className="cart-summary-value-col cart-applied-promotions" key={promotion['name']} data-el-value="checkout.appliedPromotions">&nbsp;&nbsp;{promotion['display-name']}</span>
                        );
                    })}
                </li>
            );
        ...
        if (this.props.data['_discount']) {
            return (
                <li className="cart-discount">
                    <label className="cart-summary-label-col">Today's Discount:&nbsp;</label>
                    <span className="cart-summary-value-col">{this.props.data['_discount'][0]['discount'][0]['display']}</span>
                </li>
            );
        }
    }
                ...
                <li className="checkout-tax" data-el-container="checkout.taxes">
                    <div data-region="checkoutTaxTotalRegion" style={{ display: 'block' }}>
                        <div className="checkout-tax-total">
                            <label data-el-label="checkout.taxTotal">Today's Taxes:&nbsp;</label>
                            <span data-el-value="checkout.taxTotal">{this.props.data['_order'][0]['_tax'][0]['total']['display']}</span>
                        </div>
                    </div>
                            ...
                            {this.props.data['_order'][0]['_tax'][0]['cost'].map((tax) => {
                                return (
                                    <li className="checkout-tax" key={tax['title']}>
                                        <label data-el-label="checkout.tax">{tax['title']}:&nbsp;</label>
                                        <span data-el-value="checkout.tax">{tax['display']}</span>
                                    </li>
                                );
                            })}
                        </ul>
        ...
        if (this.props.data['_order']) {
            return (
                <li className="checkout-total">
                    <label data-el-label="checkout.total">Today's Total:&nbsp;</label>
                    <span data-el-value="checkout.total">{this.props.data['_order'][0]['_total'][0]['cost'][0]['display']}</span>
                </li>
            );
        }
    }
