...
class CartCompareItems extends React.Component {
  ...
  componentDidMount() {
    const { productId, productData } = this.props;
    if (!productData) {
      login().then(() => {
        cortexFetchItemLookupForm()
          .then(() => itemLookup(productId)
            .then((res) => {
              this.onDataFetch(res);
            })
            .catch((error) => {
      ...
      this.onDataFetch(productData);
    }
  }

  componentWillReceiveProps(nextProps) {
    const { productData, productId } = nextProps;
    if (!productData) {
      login().then(() => {
        cortexFetchItemLookupForm()
          .then(() => itemLookup(productId)
        ...
        <fieldset onChange={this.handleSelectionChange}>
          ...
          <div className="guide" id={`${(Component.displayName === 'Color') ? 'product_display_item_sku_guide' : 'product_display_item_size_guide'}`} onChange={this.handleSkuSelection}>
            {Component.map((Element) => {
              const id = `selectorWeight_${Element._description[0]['display-name'].toLowerCase().replace(/ /g, '_')}-${productData._code[0].code}`;
              const name = Element._description[0]['display-name'];
              return (
                <div key={name} className={`select-wrap ${(Component.displayName === 'Color') ? 'color-wrap' : ''}`}>
                  <input key={name} type="radio" name={Component.displayName} id={id} value={(Element._selectaction) ? Element._selectaction[0].self.uri : ''} defaultChecked={name === Component.defaultChousen || Element._selectaction[0].self.uri === selectionValue} />
                  <label htmlFor={id} style={{ background: name }}>
                    {name}
                  </label>
                </div>
              );
  ...
  render() {
    ...
    if (productData) {
      ...
      return (
        <div className="item-compare-component">
          <div className="itemdetail-component container-1">
            <div>
