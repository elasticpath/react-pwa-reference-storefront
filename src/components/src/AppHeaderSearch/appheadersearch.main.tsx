    ...
    return (
      <div className={`main-search-container ${isMobileView ? 'mobile-view' : ''}`}>
        <form className="search-form" onSubmit={this.search}>
          ...
          <input className="input-search" id={`input-search${isMobileView ? '-mobile' : ''}`} type="search" onChange={this.handleChange} placeholder={intl.get('search')} ref={this.searchInput} aria-label={intl.get('search')} />
