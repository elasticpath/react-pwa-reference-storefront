...
Given('I login as following registered shopper', async (table) => {
  for (const user of table.rows()) {
    const userName = user[0];
    const password = user[1];

    await page.click('.login-btn');
    await page.type('#login_modal_username_input', userName);
    await page.type('#login_modal_password_input', password);
...
When('I navigate to the profile page', async () => {
  await page.click('#header_navbar_loggedIn_button');
  await page.click('.auth-nav-container .profile-link');
  await monitor.waitForIdle();
});

Then('I can see my purchase history', async () => {
