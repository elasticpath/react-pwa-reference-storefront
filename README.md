...
|`arKit.enable`| Optional| Boolean| The enablement toggle to load elements on the product display page for ARKit Quick Look capability. When enabled, product images that have ARKit USDZ files hosted for them will be wrapped with an anchor tag referencing the file hosted on an external CMS.|
