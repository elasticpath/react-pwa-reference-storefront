...
3. Run a Production Docker Image:
    ...
    3. On a remote host search and replace the following parameters in `docker-compose.yaml` and `nginx.conf` files:
        ...
        - Replace the `$SSL_CERT_PATH` in `nginx.conf` and `${SSL_CERT_PATH}` in `docker-compose.yaml` with the path of the certificate file on the remove server (eg. `/etc/letsencrypt/live/reference.elasticpath.com/fullchain.pem`).
        - Replace the `$SSL_KEY_PATH` in `nginx.conf` and `${SSL_KEY_PATH}` in `docker-compose.yaml` with the path of the private key on the remote server (eg. `/etc/letsencrypt/live/reference.elasticpath.com/privkey.pem`).
